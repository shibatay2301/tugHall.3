...
The "VAF.txt" file contains the Variant Allele Frequency (VAF) information of the simulation. This file is saved at the end of the simulation.
...
1. **site** -
2. **chr** - The chromosome number.
3. **gene** - The gene for which the VAF is calculated.
4. **rho** - rho is the estimation of the tumor purity.
5. **VAF_primary** - Variant Allele Frequency of the primary cells at a time step.
6. **VAF_primary_numerator** -
7. **VAF_primary_denominator** -
8. **VAF_metastatic** - Variant Allele Frequency of the metastatic cells at a time step.
9. **VAF_metastatic_numerator** -
10. **VAF_metastatic_denominator** -
...
```{r, echo=FALSE, results='asis'}
x <- read.csv(file = "../inst/extdata/Output/VAF_data.txt",header = TRUE, sep = "\t", stringsAsFactors = FALSE)
x = x[ 1 : 5, ]
knitr::kable(x, align = "c", caption = "The output of **VAF_data.txt** file. Given below is an example of the file.")

```

1. **PointMut_ID** - The point mutation ID.
2. **Parental_1or2** -
3. **Chr** - The chromosome.
4. **Ref_pos** - The reference position.
5. **Phys_pos** - The physical position.
6. **Delta** -
7. **Copy_number** - 
8. **Gene_name** - The target gene name.
9. **MalfunctionedByPointMut** - Whether the gene was malfunctioned by point mutation or not. It is shown by two logical value TRUE and FALSE.
10. **mut_order** - The mutation order.
11. **N_speckled_normal** - The total number of normal speckled cells at a time step.
12. **N_primary** - The total number of normal primary cells at a time step.
13. **N_metastatic** - The total number of metastatic cells ata time step.
14. **Copy_number_A** -
15. **N_spekled_normal_total** -
16. **N_primary_total** -
17. **N_metastatic_total** -
